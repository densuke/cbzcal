...
### 22. キャッシュ効率化と視認性の向上 (2026-03-09)

#### スマート・キャッシュ

- **包含関係によるヒット判定**: 取得済みのキャッシュ範囲内にリクエストされた期間が収まっている場合（サブレンジ）、ネットワーク通信をスキップしてキャッシュから結果を返すように改善。
- **フィルタリング**: キャッシュから読み込んだイベントを、リクエストされた具体的な期間でさらに絞り込んで表示。
- **冗長ログの抑制**: キャッシュヒット時にはバックエンドの初期化（セッション読み込み通知など）を遅延させ、実際の通信が発生したときのみ詳細情報を出すように修正。

#### 現在時刻の可視化

- **現在地マーカー**: 予定リストの中で、現在の時刻がどこに位置するかを `--- 現在 (HH:mm) ---` という形式で表示。
- **進行中予定の強調**: 現在時刻が開始〜終了時間内にある予定の行頭に `>` プレフィックスを付与。
- **実装上の工夫**: 表示ロジックに `now` パラメータを導入し、テストコードで任意の時刻における表示を検証可能に。
